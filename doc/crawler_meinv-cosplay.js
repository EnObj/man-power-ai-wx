...
function loadItems(page = 1, list = []) {
  mpaUtils.request(`http://www.win4000.com/meinvtag26_${page}.html`).then(html => {
    const $ = cheerio.load(html)
    // console.log(html)
    $('body > div.main > div > div.w1180.clearfix > div.Left_bar > div.list_cont.Left_list_cont.Left_list_cont2 > div.tab_tj > div > div > ul > li').each((index, item) => {
      list.push({
    ...
    if (page < 5) {
      ...
      loadImages(list.slice(0)).then(res=>{
        ...
        console.log(list[0])
        mpaUtils.writeToJsonFile(list, '../patch/20200810/meinv-cosplay')
      })
    }
  })
}
...
function loadImages(list) {
  ...
  return mpaUtils.request(item.link).then(html => {
    ...
    // 获取高清封面
    item.image = $('#pic-meinv > a > img').attr('data-original')
...
function loadImage(list, result=[]){
  ...
  return mpaUtils.request(item).then(html => {
    const $ = cheerio.load(html)
    result.push($('#pic-meinv > a > img').attr('data-original'))

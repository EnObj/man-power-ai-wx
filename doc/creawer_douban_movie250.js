    ...
    res.on("end", function () {
      ...
      $('#content > div > div.article > ol > li').each((index, item) => {
        ...
        list.push({
          ...
          remarks: $(item).find('div.bd > p:nth-child(1)').text().trim().split('\n').map(item => {
            return item.trim()
          }).concat([
            '评语：“' + ($(item).find('p.quote').text().trim() || '...') + '”'
          ]),
          group: 'movie-250-v5',
          radioOptions: ['看过', '想看']
        })
...
function writeToJsonFile(list) {
  // 打开目标文件
  fs.open(`/Users/jienhui/WeChatProjects/man-power-ai-wx/doc/init/mpa_content/movie-250-v5.json`, 'w', function (err, fd) {
    // 一行行写入
    var item = null

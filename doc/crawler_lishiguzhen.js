...
function loadItems(page = 1, list = []) {
  // 加载网页
  const req = http.request('http://gl.ncha.gov.cn:8080/town/find', {
    method: 'POST'
  }, (res) => {
    var html = "";
    ...
    res.on("end", function () {
      ...
      res.data.data.forEach(item => {
        // 采集信息
        list.push({
          content: item.townName,
          remarks: ["批次：" + item.batchNum, "地区：" + item.districtCode, ],
          group: 'lishiguzhen',
          radioOptions: ['去过', '想去'],
          link: 'http://gl.ncha.gov.cn/#/Industry/Famous-historical?titleType=CulturalTowns'
        })
      })
      ...
      if (page < 16) {
        ...
        console.log(list[0])
        mpaUtils.writeToJsonFile(list, '../patch/20200817/lishiguzhen')
      }
    })
  })
